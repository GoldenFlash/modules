...
class Carousel {
	constructor(options) {
		...
		var opts = Object.assign({}, optionsDefault, options);
		this.init(opts);
		this.carousel = document.querySelector(this.el);

		this.imgCt = this.carousel.querySelector(this.container);
		this.img = this.imgCt.querySelectorAll('li');

		// this.imgWidth = this.img[0].clientWidth;
		this.imgHeight = this.img[0].clientHeight;
		this.imgCtLeft = -this.imgWidth;
		this.preBtn = this.carousel.querySelector(this.btnLeft);
		this.nextBtn = this.carousel.querySelector(this.btnRight);
	...
	render() {
		var node1 = this.img[0].cloneNode(true);
		var node2 = this.img[this.img.length - 1].cloneNode(true);
		this.imgCt.appendChild(node1);
		this.imgCt.insertBefore(node2, this.img[0]);

		this.imgCt.style.width = (this.imgWidth + 2) * (this.img.length + 2) + 'px';
